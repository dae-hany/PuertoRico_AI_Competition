...
- Two tiers: **`SearchLiteAgent`** (250 nodes) and **`SearchAgent`** (1500
  nodes, the strong reference). Both beat every bundled heuristic; `Search`
  beats `SearchLite` too. Aim at `SearchLite` first, then `Search`.
...
Measured against the heuristic pool (seat-balanced; full table with sample sizes
in [BASELINES.md](BASELINES.md), regenerated by `tools/measure_baselines.py`):

| Tier | `node_budget` | vs `TradeBuilding` (best 2p heuristic) | vs `ActionValue` | typical move time |
|---|---:|---:|---:|---:|
| `SearchLiteAgent` | 250 | 96% | 88% | ~0.04 s |
| `SearchAgent` | 1500 | 100% | 100% | ~0.3 s |

`SearchLite` is the lighter tier, but "clearly beatable" describes it only next
to `SearchAgent` — it already beats every bundled heuristic. If you are looking
for a first target, beat `ActionValue` before you come for `SearchLite`.
...
lite  = SearchLiteAgent()      # lighter tier, still beats the heuristics
```
